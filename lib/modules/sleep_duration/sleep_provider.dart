...
@riverpod
class ListSleep extends _$ListSleep {
  late Dio _dio;

  int _page = 1;
  bool _hasMore = true;
  final List<SleepDurationModel> _items = [];

  @override
  Future<List<SleepDurationModel>> build() async {
    _dio = ref.read(dioProvider(ApiType.empapps));
    return _fetch(reset: true);
  }

  Future<List<SleepDurationModel>> _fetch({bool reset = false}) async {
    if (reset) {
      _page = 1;
      _hasMore = true;
      _items.clear();
    ...
    final res = await _dio.get('/sleep', queryParameters: {'page': _page});

    final data = res.data['data'];

    final List list = data['data'];

    final newItems = list.map((e) => SleepDurationModel.fromJson(e)).toList();
...
@riverpod
class UploadSleep extends _$UploadSleep {
  late final Dio _dio;
  ...
  Future<(bool, String?)> upload({
    ...
    state = await AsyncValue.guard(() async {
      ...
      await _dio.post('/sleep', data: formData);
    });
    ...
    ref.read(listSleepProvider.notifier).refresh();
