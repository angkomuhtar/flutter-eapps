...
class InspectionReportScreen extends ConsumerStatefulWidget {
  final String type;
  const InspectionReportScreen({super.key, required this.type});
...
class _InspectionReportScreenState
  ...
  void _onScroll() {
    ...
    if (_scrollController.position.pixels >=
      ...
      _hasMore = ref
          .read(inspectionReportHistoryProvider(filter: widget.type).notifier)
          .hasMore;
      if (!_hasMore) return;
      _isLoadingMore = true;
      ref
  ...
  Widget build(BuildContext context) {
    final historyAsync = ref.watch(
      inspectionReportHistoryProvider(filter: widget.type),
    );
    final hasMore = ref
        .read(inspectionReportHistoryProvider(filter: widget.type).notifier)
        .hasMore;
          ...
          child: historyAsync.when(
            data: (data) {
              ...
              return RefreshIndicator(
                onRefresh: () => ref
                    .read(
                      inspectionReportHistoryProvider(
                        filter: widget.type,
                      ).notifier,
                    )
                    .refresh(),
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return Container(
                          ...
                          child: GestureDetector(
                            onTap: () =>
                                context.push('/inspection-report/${item.id}'),
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                Container(
